...
{
    [Header("Commons")] // [Header("String")] Inspector View에 표시되는 변수들을 용도별로 구분하기 위해 사용하는 어트리뷰트  string에 작성된 내용을 굵은 글씨로 표시
    [SerializeField]
    ...
    private void EnableLaser() // 레이저 타격효과 활성
    {
        lineRenderer.gameObject.SetActive(true);
        hitEffect.gameObject.SetActive(true);
    }

    private void DisableLaser() // 레이저 타격 효과 비활성
    {
    ...
    {
        Vector3 direction = attackTarget.position - spawnPoint.position;        // 목표위치 - 레이저발사 위치로 타겟을 바라보는 방향 디렉션을 계산
        RaycastHit2D[] hit = Physics2D.RaycastAll(spawnPoint.position, direction,        //  이걸로 타겟 방향으로 레이저 발사  (레이저시작위치, 방향, 레이저 길이, 광선에 부딪히는 타겟 레이어)
        ...
        for (int i = 0; i < hit.Length; ++i)
        {
            if (hit[i].transform == attackTarget)
            {
                ...
                // 타격 효과 위치 설정
                hitEffect.position = attackTarget.position;//hit[i].point;  //attackTarget.position
